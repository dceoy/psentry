...
LABEL \
  org.opencontainers.image.title="psentry" \
  org.opencontainers.image.description="Oracle pull request sentry for Apple Container" \
  org.opencontainers.image.source="https://github.com/dceoy/psentry" \
...
RUN \
      ...
      && printf 'Binary::apt::APT::Keep-Downloaded-Packages "true";\n' \
        > /etc/apt/apt.conf.d/99psentry-keep-cache \
      && printf 'Acquire::Retries "10";\nAcquire::http::Timeout "60";\nAcquire::https::Timeout "60";\n' \
        > /etc/apt/apt.conf.d/99psentry-network-retries \
      && apt-get -yqq update \
      && apt-get -yqq upgrade \
...
COPY --chmod=0755 bin/psentry /usr/local/bin/psentry
COPY --chmod=0755 container/entrypoint.sh /usr/local/bin/psentry-entrypoint
COPY --chmod=0600 config/env.example /opt/home-skel/.config/psentry/env
COPY --chmod=0644 share/psentry/review-prompt.md \
  /usr/local/share/psentry/review-prompt.md
...
STOPSIGNAL SIGTERM
ENTRYPOINT ["/usr/bin/tini", "--", "/usr/local/bin/psentry-entrypoint"]
